...
const STEPS = [
  ...
  { n: 2, titre: "Caractéristiques", sous: "Ombrière, coupe" },
  { n: 3, titre: "Pièces du BE", sous: "Masse, coupe, façades, DP6" },
...
function etapeFaite(n) {
  ...
  if (n === 2) return Boolean(p.ombriere?.famille && p.ombriere?.puissance_kwc);
  return false;
}
...
function render() {
  ...
  if (state.etape === 2) return renderEtapeCaracteristiques(main);
  if (state.etape === 3) return renderEtapePieces(main);
...
function renderEtapeLocalisation(main) {
  ...
  main.innerHTML = `
    ...
    <details class="foldable" id="fold-mo">
    ...
    <div class="actionsrow">
      <button class="btn navy" id="btn-suivant">Continuer vers les caractéristiques</button>
    </div>`;
...
async function rechercherAdresse() {
  ...
  if (coords) {
    ...
    await positionner(coords.lon, coords.lat, { reverse: true });
...
function renderEtapeCaracteristiques(main) {
  ...
  $("#btn-suivant").addEventListener("click", async () => { await sauvegarder(); allerEtape(3); });
}

// ---- outil de mesure sur le plan de masse (calibrage + tracé) ----
const mesure = { mode: null, img: null, s: 1, pxPerM: null, seg: {}, pts: [] };
...
function renderMesure() {
  ...
  body.innerHTML = `
    ...
    <p class="sub" style="margin:8px 0 0">Astuce : calibre l'échelle en cliquant deux points d'une distance connue (une place = 2,50 m, un côté coté du plan…), puis trace la longueur puis la largeur de l'ombrière.</p>`;
  ...
  canvas.addEventListener("click", clicMesure);
...
function naturel(e) {
  ...
  return { x: (e.clientX - r.left) * (canvas.width / r.width) / mesure.s,
           y: (e.clientY - r.top) * (canvas.height / r.height) / mesure.s };
}
...
function dessinerMesure() {
  ...
  ctx.drawImage(mesure.img, 0, 0, canvas.width, canvas.height);
  const seg = (pts, color) => {
    if (!pts) return;
    ctx.strokeStyle = color; ctx.fillStyle = color; ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(pts[0].x * mesure.s, pts[0].y * mesure.s);
    ctx.lineTo(pts[1].x * mesure.s, pts[1].y * mesure.s);
    ctx.stroke();
    for (const p of pts) { ctx.beginPath(); ctx.arc(p.x * mesure.s, p.y * mesure.s, 5, 0, 7); ctx.fill(); }
  };
  ...
  if (mesure.pts.length === 1) {
    ...
    ctx.beginPath(); ctx.arc(p.x * mesure.s, p.y * mesure.s, 5, 0, 7); ctx.fill();
  }
}
...
function renderInsertionAtelier(s) {
        ...
        <div class="bd">
          <div class="field">
            ...
            <div id="ins-photo-wrap" style="${ins.photo ? "" : "display:none"}">
              <img id="ins-photo-img" style="width:100%;display:block;border-radius:8px;border:1px solid var(--gv-grey)" />
            </div>
  ...
  if (ins.photo) $("#ins-photo-img").src = urlInsertion(ins.photo);
